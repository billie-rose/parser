...
const { sortBy2, sortBy4, sortBy3 } = require('../util/sorter-util');
...
class ParsingEngine {
    ...
    parseToOutput(headerTemplate = '') {
        ...
        if (this.schema && this.schema.sortDirections) {
            this.schema.sortDirections.map(sort => {
                const copy = [...dataArr];
                copy.sort(sortBy4(sort));
                console.log(copy.map(x => x.toDelimitedString(', ')));
    ...
    sortData(dataArr, sortDirections) {
        ...
        //sortBy4(sortDirections);
        //dataArr.sort(sortBy2(sortDirections));
    }
