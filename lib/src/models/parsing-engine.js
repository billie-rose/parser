...
class ParsingEngine {
    ...
    parseToOutput(headerTemplate = '') {
        ...
        const output = sortedDataArr
            .map((data, index) => {
                ...
                return parsedData.getFormattedData().concat('\n');
            }, this)
            .join('\n');
    ...
    parseData() {
        ...
        return this.rows.map(row => {
            ...
            return new OrderedFieldObject(
                fields.map((value, index) => new Field(index, value)),
                delimeter
            );
        }, this);
    }
        ...
        if (fieldName === 'gender') {
            ...
            for (let i = 0; i < genders.length; i++) {
                if (
                    ...
                    // words for gender aren't changing anytime soon
    ...
    rearrangeFieldOrderForOutput(data) {
        ...
        data.forEach(row => {
            ...
            this.schema.outputFieldOrder.map(field => {
                newFields.push(row.getFieldByName(field));
            });
